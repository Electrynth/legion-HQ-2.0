...
class CardsContainer extends React.PureComponent {
  ...
  render() {
    ...
    */
    ...
    const filteredCards = Object.keys(allCards).sort((card1Id, card2Id) => {
      const card1 = allCards[card1Id];
      const card2 = allCards[card2Id];
      if (card1.cardName > card2.cardName) return 1;
      else if (card1.cardName < card2.cardName) return -1;
      else return 0;
    }).map((cardId) => {
